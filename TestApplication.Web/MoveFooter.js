    ...
    if (window.splitter) {
        ...
        if (__aspxIE) {
            if (__aspxBrowserVersion != 7) {
                ...
                var widestElement = FindWidestChildElement(contentPane);
                var width = widestElement.scrollWidth;
                var paddings = GetPaddings(widestElement, contentPane.parentElement);
                var leftPaneWidth = 0;
                if (!splitter.GetPaneByName("Left").isSizePx) {
                    leftPaneWidth = splitter.GetPaneByName("Left").helper.GetContentContainerElement().style.width;
                }
                else {
                    leftPaneWidth = splitter.GetPaneByName("Left").GetSize()
                }
                splitter.GetPaneByName("Content").helper.GetContentContainerElement().style.width = width + paddings;
                contentPane.style.width = width + paddings;
                if (currentSplitterHeight != newSplitterHeight) {
                    splitter.SetHeight(newSplitterHeight);
                }
                splitter.GetPaneByName("Left").SetSize(leftPaneWidth);
            }
        }
        else {
            splitter.SetHeight(newSplitterHeight);
